...
def create_requirement(
    ...
    req = JobRequirement(
        ...
        open_date=req_in.open_date or datetime.now(timezone.utc),
    ...
    audit = AuditLog(
        ...
        remarks=f"Position {req.req_code} ({req.job_title}) created for {client.name} with status OPEN."
    )
    db.add(audit)
    db.commit()
    db.refresh(req)
...
def update_requirement(
    ...
    audit = AuditLog(
        ...
        entity_id=req.id,
        old_value=old_dict,
        new_value={"position_status": req.position_status.value if req.position_status else "OPEN"},
        remarks=f"Requirement {req.req_code} ({req.job_title}) details updated."
    )
    db.add(audit)
    db.commit()
    db.refresh(req)
...
def update_position_status(
    ...
    remarks_text = status_in.remarks or f"Position {req.req_code} changed from {old_status} to {status_in.position_status.value} by {current_user.full_name}."
